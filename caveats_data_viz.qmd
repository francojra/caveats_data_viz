...
Aqui está um exemplo do discurso do State of the Union vindo do Barack Obama em 2011. Ele mostra o produto interno bruto de 2010 de 5 países, onde cada valor representa um círculo. O raio de cada círculo tem sido dimensionado baseado no tamanho da economia de cada nação.

![](images/download-10.jpeg){fig-align="center"}

Aqui, a economia dos Estados Unidos parece muito maior que as outras.

#### Correção

Isso distorce a percepção dos tamanhos relativos dos círculos, uma vez que o raio é dimensionado linearmente, mas a área em escala quadrática. Basicamente, o tamanho da economia dos Estados Unidos aparece muito maior do que deveria.

Aqui está uma versão corrigida onde o tamanho do círculo é dimensionado para a área:

![](images/download%20(1)-09.jpeg){fig-align="center"}

O Estados Unidos ainda apresenta a maior economia, mas a diferença não é tão grande em comparação aos outros países como foi feito no primeiro gráfico.

#### Conclusão

Quando trabalhar com objetos 2D, o dimensionamento deve ser feito usando a área e não o raio. Além disso, note que áreas são metáforas pobres de valores, sendo mal percebidas pelos olhos humanos. Ele deve ser usado apenas quando melhores recursos visuais já estiverem sido usados no gráfico, como o bubble plot. Neste caso, um gráfico de barras provavelmente teria feito um trabalho melhor.

### Um nota sobre rótulos longos

Na maioria das vezes, gráficos de barras e lollipop são plotados verticalmente com o eixo Y representando o valor da variável numérica. Se os rótulos da variável X são longos, eles devem ser rotacionados para que não ocorra sobreposição. Como resultado, esses rótulos se tornam difíceis de serem lidos.

![](images/download-11.png){fig-align="center"}

Nota: esse gráfico de barras mostra a quantidade de armas exportadas pelos 20 maiores países exportadores em 2017.

#### Resolvendo essa questão

A alternativa é muito simples, por que não considerar uma versão horizontal do gráfico?

![](images/download%20(1)-10.png){fig-align="center"}

#### Aviso

Note que a versão horizontal nem sempre é a melhor. Se sua variável categórica tem uma ordem natural, é melhor manter a versão vertical. Isto acontece quando:

-   você está apresentando séries temporais: tempo deve estar representado no eixo X por padrão. Não fazer isso pode confundir seu público;

-   você tem uma variável ordinal como faixa etária.

### Gráfico de barras circular e distorção

Gráfico de barras circular é uma boa alternativa ao clássico gráfico de barras. É basicamente o mesmo gráfico com cada barra ao longo de um círculo ao invés de uma linha, resultando em uma figura mais atraente.

Entretanto, os gráficos de barra circular são menos eficientes em mostrar os dados de forma acurada. De fato, é difícil comparar os comprimentos das barras desde que eles não estão na mesma linha. Além disso, a base interna das barras ficam bem menores que a base externa. Isto resulta em um viés: grupos com maiores valores aparecem bem maiores do que são na realidade.

Esse viés é ainda mais importante se:

-   o número de barras é baixo (gráfico da esquerda);

-   o raio interno do círculo é muito pequeno (gráfico da direita).

![](images/download%20(2)-08.png){fig-align="center"}

#### Resolvendo a questão

Gráfico de barras circular podem ser boas opções em condições muito específicas. Você necessita de:

-   um grande número de barras;

-   um claro padrão nos dados: se todos os grupos tem quase o mesmo valor, a versão circular não será acurada para comparação;

-   manter um raio interno maior.

![](images/download%20(3)-07.png){fig-align="center"}

Nota: não esqueça de adicionar uma legenda para as cores e uma escala para a altura das barras.

### A questão do gráfico 3D na visualização de dados

Objetos 3D são muito populares mas afetam negativamente a acurácia e velocidade em que nós podemos interpretar o gráfico na maioria dos casos. Aqui está uma revisão dos principais tipos de gráficos envolvendo 3D, com a discussão de quando ou não eles podem ser condierados uma prática ruim.

#### Gráficos de barras 3D: não

Você provavelmente tem familiaridade com esse tipo de gráfico que costuma ser criado com Excel, e assim é grandemente usado. Comparado com o clássico gráfico de barras agrupado, essa representação tem severas desvantagens:

-   é mais difícil de encontrar as porcentagens exatas das barras;

-   para valores equivalentes, as barras verdes na parte de trás parecem menores do que as barras azuis na frente;

-   algumas barras verdes de trás ficam escondidas.

![](images/download%20(4)-03.png){fig-align="center"}

[Alternativa]{.underline}: basta seguir um gráfico de barras normal.

#### Gráficos de pizza 3D: não

Nada é pior que um gráfico de pizza na visualização de dados, exceto o gráfico de pizza 3D! Adicionar 3D ao gráfico de pizza distorce ainda mais a realidade. De fato, a parte de trás do gráfico abaixo parece muito menor que a da frente, o que não reflete as porcentagens.

![](images/download%20(5)-03.png){fig-align="center"}

[Alternativa]{.underline}: não use gráfico de pizza.

#### Gráficos de dispersão 3D estáticos: não

O gráfico de dispersão 3D é um outro comum uso na visualização de dados. Esse tipo de gráfico apresenta 3 variáveis numéricas que são representadas nos eixos X, Y e Z.

A acurácia do gráfico de dispersão 3D ainda é discutida. Por exemplo, tentar determinar as coordenadas X, Y e Z de um específico ponto. Você vai perceber que é bastante impossível devido aos olhos humanos ler 3D pobremente. Assim, falta acurácia neste gráfico.

![](images/download%20(6)-03.png){fig-align="center"}

[Alternativa]{.underline}: use um gráfico de bolhas, ou um gráfico de dispersão com gradiente de cores.

#### Gráfico de dispersão 3D interativo: talvez

Em minha opinião, o gráfico de dispersão 3D pode fazer sentido apenas se você pode interagir com o gráfico. Você pode usar o mouse para mudar a orientação do gráfico e dar um zoom. Ele permite um real sentimento do gráfico 3D que não existiria sem ele. Ainda, nesse caso, é difícil obter acurácia dos pontos nas coordenadas, mas permite explorar eficientemente a estrutura dos grupos.

![](images/download%20(7)-03.png){fig-align="center"}

#### Gráfico de superfície: por que não?

Se você tem uma grade de coordenadas com variáveis numéricas em cada posição, você pode representar seus dados usando um gráfico de superfície. Esse tipo de representação faz sentido especialmente se o valor numérico representa uma altitude. Nesse caso, nós acuradamente construimos um elemento 3D que é um real objeto em 3D, fazendo mais sentido.

![](images/superficie.png){fig-align="center"}

### Cuidado com a proporção de tela

Proporção de tela é definido como a relação ou proporção entre largura e altura do gráfico. Isso pode ter um forte impacto sobre as percepções obtidas com seu gráfico.

Aqui está um exemplo com um gráfico de linhas mostrando o número de manchas solares anuais entre os anos de 1749 e 1983.

![](images/download-12.png){fig-align="center"}

O ciclo solar é bastante óbvio na figura. O número de manchas solares segue essa mudança periódica de 11 anos da atividade solar, fazendo a linha do topo para a base regularmente. Também é óbvio que alguns picos são mais altos que outros, com o máximo número de manchas solares variando de 50 a 180.

#### Brincando com as proporções

Agora vamos mudar as proporções da figura, tornando ela mais larga e menos alta. Para exagerar o efeito, o gráfico é ampliado para o período de 1800 a 1900.

![](images/download%20(1)-11.png){fig-align="center"}

As conclusões encontradas no primeiro gráfico são verdadeiras, mas aqui um novo padrão é visível. O número de manchas solares aumenta mais rápido do que diminui. Isso é devido ao fato que a taxa de decaimento das manchas é menor que a taxa de formação.

No primeiro gráfico, esse padrão não é visível desde que as linhas são quase verticais do topo para a base do eixo. No segundo gráfico, com o declive menos íngreme, o padrão é visível.

#### Alvo de 45 graus

Aqui é uma figura do W. S. Cleveland explicando que a proporção de tela de um gráfico deve ter um ângulo de 45 graus nesse tipo de gráfico. Ele mostra o que acontece quando os ângulos são muito grandes ou muito pequenos.

![](images/download%20(2)-09.png){fig-align="center"}

#### Interatividade

Nesse tipo de situação, produzir um gráfico interativo permite ao usuário dar zoom em uma parte específica e assim tentar várias diferentes proporções. Permitir esse tipo de exploração melhora bastante o valor do seu gráfico.

### A questão do empilhamento

#### O que é o empilhamento?

Empilhamento é o processo em que um gráfico é quebrado em mais de uma variável categórica que constitui o todo. Cada item da variável categórica é representada por uma área. Essas áreas são empilhadas umas sobre as outras.

Aqui está um exemplo do gráfico de área empilhada. Ele mostra a evolução dos nomes de bebês dos Estados Unidos entre 1880 e 2015. Os seis primeiros nomes mais comuns estão representados nos topos das áreas.

![](images/plot.png){fig-align="center"}

Empilhamento é uma prática comum na visualização de dados. Ele ocorre em três principais tipos de de gráficos que são altamente relacionados: gráficos de área, gráficos de barras e fluxogramas.

#### Céu ou inferno?

A eficiência dos gráficos empilhados é discutida e ele deve ser usado com cuidado. Para resumir:

-   Os gráficos empilhados são apropriados para estudar a evolução do todo e as proporções relativas de cada grupo. De fato, o topo das áreas ajuda a visualizar como o todo se comporta. No gráfico anterior, é fácil ver que em 1920, Helen e Dorothy foram nomes comuns mas os outros 4 nomes quase não existiram no mesmo período.

-   Entretanto, esses gráficos não são apropriados para estudar a evolução de cada grupo individual. Isso é devido a duas principais razões. Primeiro, alguns grupos não tem a base plana, o que torna muito difícil entender os valores dos grupos em cada período de tempo.

#### Exemplo: aritmética mental

No primeiro gráfico, tente encontrar quantas vezes o nome de Dorothy apareceu em 1920. Não é fácil encontrar os valores exatos usando esse tipo de gráfico. Você deve ter calculado mentalmente entre 25000 e 75000 de forma bem complicada. Você quer comunicar uma mensagem de forma eficiente, não que sua audiência faça uma performance aritmética.

#### Exemplo: ilusão de ótica

O Dr. Drang fornece esse exemplo bacana. Considere o gráfico abaixo, e tente visualizar como as 3 categorias estão envolvidas nesse período.

![](images/download%20(3)-08.png){fig-align="center"}

Parece óbvio que a categoria do amarelo aumentou, o roxo diminuiu, e o verde...é difícil de entender. No primeiro olhar, eu diria que parece diminuir ligeiramente.

Agora vamos plotar apenas o grupo verde:

![](images/download%20(4)-04.png){fig-align="center"}

Parece que interpretamos muito errado. Isso ocorre devido a uma ilusão de ótica. O olho humano não é bom em avaliar esse tipo de padrão visual, então esse gráfico deve ser evitado.

#### Alternativas

Se você tem poucas categorias, eu sugiro construir um gráfico de linhas. Nesse gráfico fica fácil visualizar a tendência de cada categoria ao longo do tempo e entender de forma acurada.

![](images/download%20(5)-04.png){fig-align="center"}

Entretanto, essa solução não é adequada se você tem muitas categorias. De fato, isso resultaria em um gráfico de espaguetes difícil de ler.

Ao invés, você pode usar um gráfico com pequenas múltiplas janelas. Aqui, cada categoria tem sua própria seção no gráfico. Isso torna mais fácil entender o padrão de cada categoria.

![](images/download%20(6)-04.png){fig-align="center"}

### Aritmética mental na visualização de dados

#### Exemplo

Vamos considerar o número de pessoas entrando e deixando o shopping entre 8 e 10 horas da noite. Essa é uma acurada representação usando um gráfico de linhas, que responde muito bem a questão de quantas pessoas entram e deixam o shopping.

![](images/download-13.png){fig-align="center"}

Agora, se alguém te perguntar:

-   qual a tendência do número total de pessoas no shopping?

-   que horas o número de pessoas no shopping declina?

#### Aritmética mental

Para responder as questões acima, sua audiência teria que pensar muito e isso provavelmente causaria uma confusão.

-   O marcador A indica onde o número de pessoas entrando no shopping começa a diminuir?

-   Ou marcador B indica onde mais pessoas estão deixando que entrando no shopping?

-   Ou o C é onde o número de pessoas que estão deixando o shopping diminui?

Ao invés de forçar o leitor a fazer esse cálculo, é provavelmente melhor representar o número de pessoas no shopping diretamente:

![](images/download%20(1)-12.png){fig-align="center"}

Claro, se mais pessoas deixam o shopping que entram, a quantidade total começa a diminuir (marcador B). Mas, se você quiser que a audiência foque sobre este ponto, não lhes dê trabalho extra.

#### Ligação com gráficos de empilhamento

O problema está bem relacionado a questão de empilhamento de fatores em gráficos.

### A área é uma metáfora pobre

Os olhos humanos não performam bem quando eles precisam traduzir áreas para valores numéricos. Vamos considerar as seguintes cinco bolhas. Tente ranquear elas em ordem decrescente. Você provavelmente irá concordar que isso é possível, mas toma algum tempo.

![](images/download%20(2)-10.png){fig-align="center"}

Agora vamos representar os exatos mesmos valores usando barras:

![](images/download%20(3)-09.png){fig-align="center"}

Agora ficou muito mais fácil, não é?

#### Conclusão

Isso não significa que áreas nunca deverão ser usadas para representar valores numéricos. Isso significa que outras formas e técnicas devem ser consideradas antes de usar as áreas. Por exemplo, o [gráfico de bolhas](https://www.data-to-viz.com/graph/bubble.html) faz um bom trabalho representando três variáveis numéricas.

### O gráfico de barras agrupado deve estar agrupado

O gráfico de barras agrupado exibe os valores de uma variável numérica correspondente a vários grupos. Esses grupos são agrupados através de uma variável categórica.

Quando construimos um gráfico de barras agrupado, devemos ter certeza que suas barras estão de fato agrupadas: as barras dentro de um grupo devem estar mais próximas ao mesmo grupo relacionado do que as barras de outros grupos. Neste exemplo abaixo, as barras não estão agrupadas, tornando difícil distinguir os grupos.

![](images/download-14.png){fig-align="center"}

#### Resolvendo a questão

A solução é bastante simples, basta agrupar as barras, neste caso estão agrupadas por período:

![](images/download%20(1)-13.png){fig-align="center"}

### Organizando seu gráfico

Se livrar de todos elementos desnecessários pode melhorar bastante a qualidade e impacto do seu gráfico. Primeiro, o gráfico ficará mais claro e assim mais fácil de ser interpretado pelas pessoas. Segundo, ele irá permitir as pessoas atingir o alvo da principal informação do gráfico.

#### Gráfico inicial

A ideia desse gráfico é mostrar que as mulheres tendem a fazer compras de natal mais cedo do que os homens:

![](images/download%20(2)-11.png){fig-align="center"}

#### Aparência final

![](images/download%20(3)-10.png){fig-align="center"}

#### Passo a passo

Aqui estão os componentes que você pode remover ao construir seu gráfico:

-   3D (sempre);

-   Efeitos de cores para decoração (sempre);

-   Linhas de grade (se não forem necessárias);

-   Eixos (se não forem necessários);

-   Decimais;

-   Brique com as fontes e tamanhos de textos: alguns textos são mais importantes que outros.

Aqui está uma animação mostrando a evolução do primeiro gráfico para o último, seguindo os passos para melhorá-lo.

![](images/download.gif){fig-align="center"}

### Consistência entre gráficos

Vamos considerar um pequeno relatório onde você apresenta vários gráficos para sua audiência. O relatório é composto por:

-   um gráfico de barras mostrando a quantidade total de dinheiro gasto sobre cinco produtos;

-   um gráfico de linhas mostrando a evolução do dinheiro gerado por esses cinco produtos nos últimos dez anos.

![](images/download%20(4)-05.png){fig-align="center"}

Aqui, é muito difícil fazer uma ligação entre os dois gráficos apesar deles estarem relacionados, devido a paleta de cores deles ser diferente. O produto A no gráfico da esquerda está em uma cor roxa escura e no gráfico da direita em uma cor vermelha. Isso leva tempo para fazer uma ligação entre os gráficos.

#### Seja consistente

Quando você tem vários gráficos relacionados, use a mesma cor para os mesmos itens ao longo do relatório.

![](images/download%20(5)-05.png){fig-align="center"}

### O gráfico de aranha e seus alertas

#### Definição

O gráfico de aranha ou gráfico de radar é um tipo de gráfico bi-dimensional desenhado para representar uma ou mais série de valores ao longo de múltiplas variáveis quantitativas. Cada variável tem seu próprio eixo e todos os eixos são unidos no centro da figura.

Vamos considerar os resultados de um exame de um estudante. Ele tem marcado pontuações de 0 a 20 para dez tópicos como matemática, esportes, estatística, etc. O gráfico de radar promoveu um eixo para cada tópico. A forma do gráfico permite você ver quais tópicos o estudante performou melhor ou pior.

![](images/download%20(6)-05.png){fig-align="center"}

#### Variação

No gráfico anterior, apenas uma série é plotada, mostrando como um estudante performou. Uma tarefa comum é comparar vários indivíduos. Com poucas séries é possível mostrar cada grupo no mesmo gráfico. Aqui é bastante óbvio que Shirley globalmente performou melhor que Sonia, exceto em Sport, English e R-coding.

![](images/download-15.png){fig-align="center"}

Com mais que duas ou três séries, é uma boa prática usar múltiplas pequenas janelas para evitar sobreposição dos dados e desordem na figura. Cada estudante fica representado em um individual gráfico radar. Assim fica fácil entender as características de um indivíduo em específico, e observar por similaridades nas formas para encontrar estudantes com características parecidas.

![](images/download%20(1)-14.png){fig-align="center"}

#### Então qual o erro com esses gráficos?

Os gráficos de aranha costumam ser crticiados por:

-   *Layout circular = mais difíceis de ler* - Valores quantitativos ficam muitos mais fáceis de serem comparados quando eles estão sulcados ao longo de um simples eixo vertical ou horizontal. Essa abordagem também serve para outros layouts circulares. Compare as figuras abaixo que consideram os dados de apenas um estudante. Fica mais fácil de comparar valores em um gráfico de barras, e mais acuradamente.

![](images/download%20(2)-12.png){fig-align="center"}

![](images/download%20(3)-11.png){fig-align="center"}

-   *Suporte para ranqueamento* - No exemplo acima, o gráfico de pirulito (lollipop) está ordenado. Isso permite você ver instantaneamente quais tópicos apresentam a melhor e pior marcação. Isso é mais difícil de perceber nos gráficos de aranha onde os tópicos não iniciam nem terminam.

-   *Ordem das categorias influenciam no impacto da visualização* - Os leitores dos gráficos de aranha irão provavelmente focar na forma observada. Isso pode confundir e distrair o leitor devido as diferentes formas altamente dependerem da ordem das categorias em torno do gráfico. Veja esses gráficos que apresentam os mesmos dados mas ordens das categorias distintas.

![](images/download%20(4)-06.png){fig-align="center"}

-   *Escalas* - Os gráficos de aranha exibem os valores de várias variáveis quantitativas, todas representadas sobre um eixo. Nos exemplos, a mesma escala e mesmas unidades são compartilhadas por todas as variáveis (ranking de 0 a 20). Também é possível exibir variáveis que são completamente diferentes. Nesse caso, não esqueça de mostrar uma escala óbvia para cada, caso contrário, o leitor esperará a mesma escala.

-   *Sobreplotagem* - apresentar mais que duas séries, pode tornar o gráfico inelegível. Evite isso e use pequenas múltiplas janelas para cada grupo.

-   *Super avaliação das diferenças* - a área da forma nos gráficos de aranha também aumenta quadraticamente mais que linearmente, que pode levar aos leitores pensar que pequenas mudanças são maiores do que elas realmente são. No seguinte exemplo, o estudante da esquerda tem o score de 7 em casa tópico. Por sua vez, o estudante da direita tem o score de 14 em cada tópico. Entretanto, a área da figura da direita parece ser mais que duas vezes maior que a área da figura da esquerda.

![](images/download%20(5)-06.png){fig-align="center"}

#### Alternativas

-   Se você tem apenas uma séries de dados para exibir e todas são de variáveis quantitavivas com a mesma escala, então use um gráfico de barras ou de pirulito e ranqueie as variáveis.

![](images/download%20(6)-06.png){fig-align="center"}

-   Se você tem duas séries, você ainda pode brincar com o gráfico de barras ou pirulito. Esse gráfico abaixo foca no primeiro estudantes com círculos azuis escuros e permite comparar os dados com um outro estudante (círculos azuis claros).

![](images/download%20(7)-04.png){fig-align="center"}

-   Se você tem algumas poucas séries, use o facetamento com gráficos de barras ou de pirulito para ajudar a resolver o problema.

![](images/download%20(8)-02.png){fig-align="center"}

-   Se você tem muitas séries, ou se suas variáveis não tem a mesma escala, então a melhor opção provavelmente é mudar para um gráfico de coordenadas paralelas.

![](images/download%20(9)-01.png){fig-align="center"}

### Heatmap

#### Definição

Um heatmap é uma representação gráfica de dados onde valores individuais contidos em uma matriz são representados através de cores. É um pouco como olhar uma tabela de dados.

Aqui está um exemplo mostrando oito características gerais, como população e expectativa de vida, para cerca de 30 países em 2015.

![](images/Sem%20t%C3%ADtulo.png){fig-align="center"}

#### Para que serve?

Heatmap é realmente útil para exibir uma visão geral de dados numéricos, não para extrair informações específicas dos dados. No gráfico acima, é possível verificar o grande tamanho populacional da índia e da China, por exemplo.

Heatmap também é útil para exibir resultados de agrupamentos hierárquicos. Basicamente, os agrupamentos ajudam a verificar quais países são similares. O caminho comum para representar esses resultados é usar os dendogramas. Esses dendogramas podem ser desenhados no topo do heatmap.

![](images/g2.png){fig-align="center"}

Neste gráfico, Afeganistão, índia e Bolívia estão agrupados juntos. De fato, estes três países apresentam forte expansão, com muitas crianças por mulheres, mas alta taxa de mortalidade.

Nota: neste heatmap, as características também são agrupadas. Por exemplo, expectativa de vida e mortalidade estão agrupadas juntos devido a elas serem altamente correlacionadas.

#### Variação

-   Nós vimos anteriormente que o heatmap é frequentemente usado para exibir os resultados de um algoritmo de agrupamento. Uma tarefa comum é comparar os resultados com as expectativas. Por exemplo, nós podemos checar se os países estão agrupados de acordo com o continentes deles usando barras coloridas.

-   Para heatmap estáticos, uma prática comum é exibir os valores exatos de cada célula em números. De fato, é difícil traduzir uma cor em um número preciso.

-   Heatmaps podem também ser usados para séries de tempo, onde existe um padrão regular no tempo.

-   Heatmaps podem ser aplicados à matrizes de adjacência, como esta abaixo.

![](images/1_gny0Hz4hd90zDeCyY0s72A.png){fig-align="center"}

#### Erros comuns

-   Frequentemente é necessário normalizar os dados;

-   Na análise de agrupamento, é necessário permutar as linhas e colunas da matriz para colocar valores semelhantes próximos uns dos outros de acordo com o agrupamento;

-   A paleta de cores é importante.

### Anotação é crucial para visualização de dados

Anotação é um componente crucial de uma boa visualização de dados. Isso pode mudar de um gráfico chato para um gráfico com uma interessante e perspicaz informação. Visualização de dados é frequentemente separado em dois principais tipos: análise exploratória e explicativa. A anotação é usada para o primeiro tipo.

Nossa atual atenção para olhar coisas online é, na média, menos que cinco segundos. Então, se você não pode prender a atenção de alguém em cinco segundos, você provavelmente irá perder o leitor. Adicionar uma anotação acurada pode ajudar bastante a segurar a atenção da sua audiência. Use palavras chaves, formas, cores e outros visuais para ajudar eles a irem direto ao ponto.

Anotação é um conceito geral, e existem centenas de formas para adicionar anotações em um gráfico, dependendo do contexto. Vamos ver alguns exemplos.

#### Exemplo 1

O principal perigo de um gráfico de linhas é acabar como um gráfico de espaguete: muitos grupos nesse tipo de gráfico, resulta em uma figura confusa onde é difícil identificar padrões.

![](images/download-16.png){fig-align="center"}

Ao apresentar esse gráfico para a sua audiência, você provavelmente vai querer destacar um ponto principal. Vamos dizer que você esteja interessado em especificar a evolução do nome 'Amanda'. Então, faz mais sentido construir algo como isto:

![](images/download%20(1)-15.png){fig-align="center"}

#### Exemplo 2

Aqui você pode ver que a anotação apresenta um papel chave:

-   a linha que representa Blackpool é ligeiramente mais grossa, então ela se destaca um pouco mais;

-   os histogramas são interessantes por causa das anotações. Note que a cor é consistente entre os gráficos;

-   os valores das lacunas no início e no final da linha são escritos para que o leitor não precise fazer as contas de cabeça.

![](images/download-17.jpeg){fig-align="center"}

### Barras embrulhadas de Stephen Few: reflexões

Stephen Few apresentou essa semana outras formas inovadoras de visualizar dados: barras agrupadas. Cheque esse artigo parama explicaçã do que elas sãodo porque que elas são úteis. Neste post eu quero discutor minhas reflexões sobre elas.

1.  **O medo do novo**

Stephen criou os gráficos de barras embrulhadas para consertar problemas com treemaps. Eu confesso que a primeira vez que vi os gráficos de barras embrulhadas, eu não gostei delas. Mas foi isso o medo do novo? Eles claramente consertam um problema com os treemaps, mas inicialmente eu pensei, "Bem, treemaps são bons o suficiente, por que nós necessitamos de mais algo para consertar um pequeno problema?

Mas e se Ben Schneiderman não inventasse as barras embrulhadas, e sim os treemaps na década de 1990? Estaríamos confortáveis com isso, agora, eu suspeito. E então imagine que Stephen Few apareça e resolva um problema com Wrapped Bars, fornecendo-nos o mapa da árvore. Será que minha reação ainda seria que o novo é inferior? Eu suspeito que sim. Isso é algo que sempre deve estar em mente quando avaliamos novos tipos de visualização.

Com aquilo em mente. qual é o ponto ideal que as barras embrulhadas consertam? Em essência, trata-se de encontrar a magnitude relativa exata dos membros da dimensão nos níveis mais baixos. Por exemplo, qual é a classificação da marca vermelha no treemap abaixo?

![](images/treemap-with-colour.png)

É muito difícil responder isso. Agora cheque o gráfico de barras embrulhadas com 60 linhas para cada coluna.

![](images/wrapped-with-colour.png)

Ficou muito mais fácil, não é? Eu estou interessado em saber o quão importante isso é para os usuários de ferramentas de visualização. Acho que nunca senti necessidade de um meio-termo entre o mapa da árvore e o gráfico de barras. Alternar entre os dois foi suficiente para mim até agora.

2.  **Padrão Moire**

O gráfico acima cria um padrão Moire que perturba meus olhos quando eu escaneio o gráfico. Eu recomendaria usar o lollipop para gráficos de barras embrulhadas: eles reduzem a quantidade de tinta enquanto preserva a ligação do ponto de volta ao eixo de origem. Aliás, o lollipop só deve ser usado quando a escala começa do zero. Se estiver mostrando um outro valor, então a linha deve se estnder por toda a área do gráfico.

![](images/wrapped-with-colour-lollipop1.png)

3.  **Valores negativos**

Stephen aborda valores negativos e fornece um exemplo onde um pequeno número de valores são negativos. Isso funciona bem o suficiente e é uma melhoria dos treemaps. Deveria ser um exercício interessante para ver como ele funciona se os dados apresentam 50% de valores negativos.

4.  **Minhas conclusões**

O gráfico de barras embrulhadas tem vantagens definitivas sobre as visualizações de dados existentes, mas temo que, sejam quais forem, este tipo de visualização demorará a ser adotado.

Espero que haja mais sucesso. Há momentos durante um ciclo de análise visual em que você deseja compreender os membros de classificação inferior de uma dimensão, e acredito que a barra agrupada pode ter mais valor nesse caso.

### Como ler uma escala logarítmica

A poucas semanas atrás, eu li um tweet do Mike Bostock que dizia **"não compare a mudança de porcentagem em uma escala linear, use uma escala log ao invés disso. -50% (0,5×) é uma mudança tão grande quanto +100% (2×)."** Ele inseriu dois mapas com os mesmos dados, mas um deles estava em escala linear (primeiro gráfico) enquanto o outro estava em escala logarítmica (segundo gráfico).

![](images/full-180614_log2-1-1536x995.png){fig-align="center"}

![](images/full-180614_log1-1-1536x995.png){fig-align="center"}

Eu cocei minha cabeça. Por que -50% é tão grande quanto 100%? Eu não sou um gênio da matemática, então levei algum tempo para entender tudo. Neste gráfico exploraremos o que o autor do tweet quis dizer - e quando não seguir seus conselhos.

*Mike desenterrou os valores absolutos desses dados e publicou [esta excelente explicação](https://observablehq.com/@mbostock/methods-of-comparison-compared) sobre as diferentes maneiras de comparar dois valores entre si em um mapa. Basicamente, você pode parar de ler esta postagem do blog e ir para a dele.*

Vamos imaginar um condado com quatro pessoas com trastorno por uso de álcool em 1980. Em 2014, esse número caiu para duas pessoas. Pode-se dizer que o número de 1980 **caiu pela metade**, ou que diminiu 50%, ou pode-se dizer que o número é 0,5x maior (já que 4 × 0,5 = 2; é o mesmo que 4 ÷ 2).

Vamos imaginar um segundo condado em que o número de 1980 de quatro pessoas **duplicou** para oito casos em 2014. Esse aumento foi de 100%. O número é agora duas vezes maior (4 x 2 = 8).

Então por que Mike Bostock declarou que 50% e 100% apresentam a mesma mudança? **Porque a mudança não é na porcentagem, mas na taxa**. A mudança na porcentagem para os condados é -50% e 100%. Mas a mudança na taxa é x2 e ⁒2. A diferença entre 2 e 4, e a diferença entre 4 e 8 é a mesma para a taxa de crescimento.

E isso é o motivo do Mike querer mostrar esses números na escala log. **Apenas sobre uma escala log, as mudanças nas mesmas taxas são mostradas com as mesmas distâncias.**

![](images/full-180614_log-1536x552.png){fig-align="center"}

Nós podemos calcular isso. Você lembra da fórmula do log~10~x usada na escola? Nós podemos colocar nossos **"0,5x"** e **"2x"** nela. Se nós fazemos isso, a porcentagem muda de -50% (= 0,5x) em nosso primeiro condado mostrando uma distância de **-0,301 (=log~10~0,5)**. Já o aumento de 100% no nosso segundo condado irá mostrar uma distância de **0,301 (=log~10~2)**. Boom! -0,301 e 0,301 estão à mesma distância de zero.

Aqui estão todas as mudanças nas porcentagens que nós podemos ver no mapa, mas mostrado em um gráfico de dispersão que compara a porcentagem linear com os valores de log.

#### ![](images/log.png)

Então o Mike Bostock está certo? Nós não deveríamos mais mostrar mudanças nas porcentagens sobre a escala linear?

Bem, isso depende do que nós queremos mostrar. (Sim, eu sei que isso é a resposta de 90% das questões relacionadas a gráficos, mas ele é simplesmente verdade 90% das vezes).

**Se você quer mostrar como quantas pessoas são afetadas, nós devemos considerar mostrar essas mudanças nas porcentagens em uma escala linear**. Se nós mostramos as mudanças de pessoas com disordem no uso de álcool entre 1980 e 2014 em nossos primeiro e segundo condados na escala log, eles irão ser mostrados com a mesma distância de 0,301. Mas o número de pessoas afetadas é diferente: no primeiro condado, 2 pessoas a menos foram afetadas. No segundo condado, 4 pessoas a mais foram afetadas. Numa escala linear, a distância entre 2 e 4 será duas vezes maior. A mesma distância/unidade representa o mesmo número de pessoas (se a base numérica é a mesma). Então, por exemplo, um gráfico de desemprego, mostrando mudanças percentuais em uma escala linear, faz sentido.

Mas existem gráficos onde **a taxa de crescimento é mais importante que a mudança na porcentagem**. Preços das ações são bons exemplos, desde que a mudança no preço é mais importante que o aumento no valor absoluto do dólar.

#### Então o que deveríamos fazer de diferente na próxima vez que construirmos mapas?

Nem tanto, na verdade. Mapas são casos especiais: mais escuro significa mais e mais claro significa menos; mas é impossível para um ser humano normal ler os números subjacentes em um mapa apenas com base nas cores.

Além disso, nosso número de paradas, o tipo de paradas e as cores específicas que usamos influenciam a forma como os dados serão percebidos.

*Como tantas vezes no design de gráficos, vem abaixo o senso comum, pensar sobre o que nós queremos mostrar e a atitude geral para mostrar os dados com veracidade. Sabemos a diferença entre escala log e escala linear, e isso pode nos ajudar a fazer melhores escolhas na produção dos gráficos. Mas como regra geral, a escala linear irá trabalhar melhor na maior parte das vezes, especialmente quando nós estamos apresentando dados sobre pessoas.*
